...
$directory = "../../../files/media/images"; // Use your correct (relative!) path here
...
if (is_dir($directory)) {
    ...
    while ($file = readdir($direc)) {
		$check_result=preg_match('~^.~', $file);
        if ($check_result!==false && $check_result>0) { // no hidden files / directories here...
             if (is_file("$directory/$file") && getimagesize("$directory/$file") != FALSE) {
                ...
                $output .= $delimiter
                    ...
                    . utf8_encode(str_replace("../../../","",$directory)."/$file")
                    . '"],';
