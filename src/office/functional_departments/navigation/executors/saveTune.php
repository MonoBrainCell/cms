...
class navElemsTextRedact{
	...
	public function __construct(){
		...
		||
		($_POST['type_of_tune'][0]!="main" && $_POST['type_of_tune'][0]!="advanced" && $_POST['type_of_tune'][0]!="relatedElems")
		){
	...
	private function initCheckArray($type){
		if ($type=="main")
			$returnArray=array(
				"designedFile"=>self::MAIN_NAVIGATION_TPL_FILE,
				"elements"=>array(
						"navigation_wrapper",
						array(3,array("direction_wrapper","anchor"))
					)
				);
		else if ($type=="advanced")
			$returnArray=array(
					"designedFile"=>self::ADV_NAVIGATION_TPL_FILE,
					"elements"=>array("navigation_wrapper","anchor")
				);
		else if ($type=="relatedElems")
			$returnArray=array(
					"designedFile"=>self::RELELEM_NAVIGATION_TPL_FILE,
					"elements"=>array("path_wrapper","path_anchor","close_relatives_wrapper","close_relatives_anchor")
				);
		return $returnArray;
	}
}
