...
class App_news_newsList extends ErrorsManager{
	const NAVIGATION_FILE="../functional/news/newsList.fbd.csv";
	const CONTENT_TPL_PATH="functional_departments/news/dummyes/newsList_mainContent.fbd.tpl";
	const ELEM_ROW_TPL_PATH="functional_departments/news/dummyes/newsList_elements_row.fbd.tpl";
	const ELEM_TPL_PATH="functional_departments/news/dummyes/newsList_element.fbd.tpl";
	
	const NUMERAL_ANCHOR_TPL="<a href='office/index.php?dep=news&trunk=newsList.php&group={PAGE_NUMBER}'>{PAGE_NUMBER}</a>";
	...
	public function __construct(){
		...
		if (file_exists(self::ELEM_TPL_PATH)===true)
			...
			$elemTemplate=false;
		if ($navigation===false){
			$this->content="<h1>Файл newsList.fbd.csv, расположенный в functional/news повреждён или отстутствует.</h1>";
			return false;
		}
		else if (empty($navigation)===true){
			$this->content="<div id='main_content'><h1>Вы не добавили ни одной новости.</h1><a class='main_functions' href='office/index.php?dep=news&branch=redactContent.php&newsId=new'>Добавить новость</a>
			<a class='main_functions' href='office/index.php?dep=news&branch=newsTuneList.php'>Настроить новостной блок</a></div>";
			return false;
		}
			...
			for ($a=$numeralAddon['startPosition'],$c=1;$a<$numeralAddon['endPosition'];$a++){
				...
				$tempString.=str_replace("{NEWS_NUMBER}",$structureElems[0],$elemTemplate);
				$tempString=str_replace("{NEWS_DATE}",$structureElems[1],$tempString);
				$tempString=str_replace("{NEWS_HEADER}",$structureElems[2],$tempString);
			...
			$this->content=str_replace("{REPLACEMENT_PART}","<h1>Список добавленных новостей</h1>"."<div class='pages_navigation first_block'>{$numeralAddon['numeralNavigation']}</div>".$tempContent,$this->content);
		}
	}
