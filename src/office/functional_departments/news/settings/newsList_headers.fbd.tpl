...
$(document).ready(function(){
	$('.remove_datas').click(function(){
		var pageName=$(this).parent().siblings(".element_string").children("em").children("a").text();
		var deleteAgree=confirm("Вы уверены, что хотите удалить новость '"+pageName+"'? Весь текстовый контент используемый в ней будет удалён безвозратно.");
		if (deleteAgree===true){
			...
			datas.phpFileInc="functional_departments/news/executors/deletePage.php";
