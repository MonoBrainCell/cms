...
class deletePage extends ErrorsManager {
	...
	const NAVIGATE_BUTTONS="<a class='buttons_from_script' href='office/index.php?dep=filesShare&trunk=filesList.php'>Вернуться к списку файлов</a>\r\n<a class='buttons_from_script' href='office/index.php'>Вернуться на главную</a>";
	private $fileId=0;
	private $strNum=0;
	...
	public function deleteStartEngine(){
		if (file_exists(self::NAVIGATION_FILE)===false){
			echo "<span class='error_class'>Не найден файл для записи информации о созданных списках. Скорее всего он был потерен из-за системного сбоя сервера. Обратитесь к Вашему Хостинг-Провайдеру для проведения backUp'а Вашего сайта.</span>\r\n".self::NAVIGATE_BUTTONS;
			exit;
		}
	...
	private function rewriteNavigationFile(){
		...
		if (@file_put_contents(self::NAVIGATION_FILE,$content)===false){
			echo "<span class='error_class'>Произошёл сбой при записи файла списка созданных списков файлов для скачивания! Проверьте права на возможность перезаписи, данные файлу filesList.fbd.csv, находящемуся в functional/filesShare/ .</span>\r\n".self::NAVIGATE_BUTTONS;
			exit;
		}
	}
}
