...
function prepareToSendDatas(ajaxWindowId){
	if ($("#"+ajaxWindowId).find(".success_class").length>0){
		...
		$("form#ajax_tmpr_block").attr("action","office/index.php?dep=loadFiles&trunk=loadFiles.php");
...
$(document).ready(function(){
	$("body").on('click',".abort_loading_files",function(){
		...
		$("form[target=ajax]").attr("action","functional_departments/loadFiles/executors/checkFilesOnSite.php");
