...
class App_costEngine_keeper extends ErrorsManager {
	const CONTENT_SAVE_PATH="../functional/costEngine/content/occContent.fbd.tpl";
	const HEADERS_SAVE_PATH="../functional/costEngine/content/occHeaders.fbd.tpl";
	const NAVIGATE_BUTTONS="<span id='ajax_close_button'>Вернуться к редактированию формы калькулятора</span>\r\n<a class='buttons_from_script' href='office/index.php'>Вернуться на главную</a>";
	...
	public function startKeeperEngine(){
		...
		if (mb_strlen(trim($_POST['body_code'][0]))>0)
			...
			echo "<span class='error_class'>Вы оставили пустым поле <q>Html-код формы калькулятора</q>!</span>\r\n".self::NAVIGATE_BUTTONS;
			exit;
		}
	...
	private function reloadCodeFile($fileCode,$path) {
		...
		if (@file_put_contents($path,$fileCode)===false){
			echo "<span class='error_class'>Произошёл сбой при записи кода формы! Проверьте права данные файлам в папке functional/costEngine/content на возможность их перезаписи.</span>\r\n".self::NAVIGATE_BUTTONS;
			exit;
		}
	}
}
