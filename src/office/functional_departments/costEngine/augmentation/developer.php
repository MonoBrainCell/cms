...
class App_costEngine_developer {
	const FEEDACK_DEV_CONTEINER="functional_departments/costEngine/dummyes/redactCostEngine_conteiner.fbd.tpl";
	const FEEDBACK_CONTENT_PATH_PREFIX="../functional/costEngine/content/";
	const FEEDBACK_HEADERS_PREFIX="occHeaders.fbd.tpl";
	const FEEDBACK_BODY_PREFIX="occContent.fbd.tpl";
	private $htmlContent;
	...
	private function genContent(){
		...
		if (file_exists(self::FEEDBACK_CONTENT_PATH_PREFIX . self::FEEDBACK_HEADERS_PREFIX)===true)
			...
			$this->htmlContent="<div id='main_content'><h1>Не найден файл с рабочими заголовками для браузера. Проверьте его наличие по указанному пути functional/costEngine/content/". self::FEEDBACK_HEADERS_PREFIX ."</h1></div>";
			return false;
		}
		if (file_exists(self::FEEDBACK_CONTENT_PATH_PREFIX . self::FEEDBACK_BODY_PREFIX)===true)
			...
			$this->htmlContent="<div id='main_content'><h1>Не найден файл с html-кодом формы калькулятора. Проверьте его наличие по указанному пути functional/costEngine/content/". self::FEEDBACK_BODY_PREFIX ."</h1></div>";
			return false;
		}
	}
